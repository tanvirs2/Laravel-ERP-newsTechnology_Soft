...
@foreach ($employees as $employee)
    <tr class="orderRow" id="row" orderId="" style="">
        ...
        <td class="text-center blackCol">
            {{ $employee->prSwOut*$employee->unit_price }}
            {{--*/ $prSwInVal += $employee->prSwOut*$employee->unit_price /*--}}
        </td>
        <td class="text-center blackCol">
            {{ round(($employee->cmPerDz*$employee->prSwOut)/12) }}
            {{--*/ $prCm = ($employee->cmPerDz*$employee->prSwOut)/12 /*--}}
            {{--*/ $prCmSum += $prCm /*--}}
        </td>
        <td class="text-center blackCol">
            {{ $employee->smv * $employee->prSwOut }}
            {{--*/ $totSMV += $employee->smv /*--}}
        </td>
        <td class="text-center blackCol">
            {{ $employee->prIron}}
